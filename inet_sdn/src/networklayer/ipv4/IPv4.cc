...
void IPv4::sendPacketToNIC(cPacket *packet, const InterfaceEntry *ie)
{
    ...
    // <F.R.>
    // fixed get gate index
   /*
    std::string str = "queueOutBaseId: ";
    str.append(std::to_string(queueOutGateBaseId));
    str.append("---");
    str.append("gateIndex: ");
    str.append(std::to_string(ie->getNetworkLayerGateIndex()));
	opp_error(str.c_str());
	
	
	int networkLayerGateIndex = ie->getNetworkLayerGateIndex();
	// if virtual interface
	if (networkLayerGateIndex == -1) {
		send(packet, queueOutGateBaseId);
	}
	else {
		send(packet, queueOutGateBaseId + networkLayerGateIndex);	
	}
	*/
